...
public class ECUxDataset extends Dataset {
    ...
    private final double hpa_per_psi = 0.0145;
	...
	*/
	...
	/*****************************************************************************/
		...
		if(c==null) {
		    ...
		    /*Calc PSI*/
		    if (id.toString().endsWith(" (PSI)")) {
				String s = id.toString();
				s = s.substring(0, s.length()-6);
				final Column t = this.get(s);
				if (t!=null) {
				    final DoubleArray data = t.data;
				    final String units = t.getUnits();
				    c = new Column(id, "(PSI)", toPSI(data,units));
				}
		    }
		}
    ...
    private DoubleArray toPSI(DoubleArray data, String units) {
    	final Column atmosPress = this.get("BaroPressure");
    			
    	if (units.equals("hPa"))
			return data.sub(atmosPress.data).mult(hpa_per_psi);
    	else if (units.equals("mBar"))
    		return toPSI(data);
		else
			return data;
	}
